...
You can create a workspace, rename it, switch to a named workspace.
You can go to the previous or next workspaces, or you can go to the
workspace number (0-9).
...
Workspaces actually consist of two workspaces.  The base workspace,
and the working workspace.  The base config is the one that you have
